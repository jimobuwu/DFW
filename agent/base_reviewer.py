...
class BaseReviewer:
    ...
    @staticmethod
    def _build_skill_hint(candidate: dict) -> str:
        """将候选股票的 skill 量化评分组装为 AI 复评的提示文本。"""
        extra = candidate.get("extra", {})
        if not extra:
            return ""
        parts = []
        if "vol_price_score" in extra:
            parts.append(f"量价结构预评分={extra['vol_price_score']}/5")
        if "trend_score" in extra:
            parts.append(f"趋势强度预评分={extra['trend_score']}/5")
        if "abnormal_score" in extra:
            parts.append(f"建仓异动预评分={extra['abnormal_score']}/5")
        if "skill_total" in extra:
            parts.append(f"综合预评分={extra['skill_total']:.1f}")
        if not parts:
            return ""
        return (
            "\n\n【量化预评分参考（基于日线指标计算，仅供参考，请结合图表独立判断）】\n"
            + "、".join(parts)
        )
    ...
    def run(self):
        ...
        for i, candidate in enumerate(candidates, 1):
            ...
            skill_hint = self._build_skill_hint(candidate)
            augmented_prompt = self.prompt + skill_hint
            ...
            try:
                ...
                if candidate.get("extra"):
                    result["skill_scores"] = candidate["extra"]
