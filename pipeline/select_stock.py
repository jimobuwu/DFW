...
职责：
  ...
  - 计算 skill 质量分（量价结构、建仓异动、趋势强度）附加到 Candidate.extra
...
# =============================================================================
# Skill 质量评分（基于 B1 技能知识）
# =============================================================================

def _compute_skill_scores(df: pd.DataFrame, pick_date: pd.Timestamp) -> Dict[str, float]:
    """
    根据 B1 skill 知识计算量化质量分，用于 AI 复评的辅助信息。

    返回 dict 包含：
      - vol_price_score: 量价结构分（1~5）
      - trend_score:     趋势强度分（1~5）
      - abnormal_score:  建仓异动分（1~5）
      - skill_total:     加权总分
    """
    if pick_date not in df.index:
        return {}

    loc = df.index.get_loc(pick_date)
    if not isinstance(loc, int):
        loc = int(loc) if np.isscalar(loc) else 0

    lookback = min(loc + 1, 60)
    window = df.iloc[loc - lookback + 1: loc + 1]
    if len(window) < 10:
        return {}

    close = window["close"].to_numpy(dtype=float)
    volume = window["volume"].to_numpy(dtype=float) if "volume" in window.columns else None
    open_ = window["open"].to_numpy(dtype=float) if "open" in window.columns else None

    scores: Dict[str, float] = {}

    # --- 1) 量价结构分 ---
    vp = 3.0
    if volume is not None and open_ is not None and len(close) >= 5:
        recent = min(len(close), 20)
        rc = close[-recent:]
        rv = volume[-recent:]
        ro = open_[-recent:]
        ...
        up_vol = rv[1:][up_mask].mean() if up_mask.any() else 0
        dn_vol = rv[1:][dn_mask].mean() if dn_mask.any() else 0
        ...
        max_vol_idx = int(np.argmax(rv))
        if rc[max_vol_idx] < ro[max_vol_idx]:
            vp = max(vp - 1.5, 1.0)

    scores["vol_price_score"] = round(vp, 1)

    # --- 2) 趋势强度分 ---
    ts = 3.0
    if "zxdq" in window.columns and "zxdkx" in window.columns:
        zxdq_last = float(window["zxdq"].iloc[-1])
        zxdkx_last = float(window["zxdkx"].iloc[-1])
        close_last = close[-1]

        if np.isfinite(zxdq_last) and np.isfinite(zxdkx_last) and zxdkx_last > 0:
            above_pct = (close_last - zxdkx_last) / zxdkx_last
            zxdq_spread = (zxdq_last - zxdkx_last) / zxdkx_last

            if above_pct > 0 and zxdq_spread > 0:
                if above_pct < 0.05 and zxdq_spread < 0.03:
                    ts = 5.0
                elif above_pct < 0.10:
                    ts = 4.0
                elif above_pct < 0.20:
    ...
    if "wma_bull" in window.columns and bool(window["wma_bull"].iloc[-1]):
        ts = min(ts + 0.5, 5.0)

    scores["trend_score"] = round(ts, 1)

    # --- 3) 建仓异动分 ---
    ab = 3.0
    if volume is not None and len(close) >= 20:
        lookback_ab = min(len(volume), 40)
        vols = volume[-lookback_ab:]
        cls = close[-lookback_ab:]
        avg_vol = vols.mean()

        surge_mask = vols > avg_vol * 2.0
        if surge_mask.any():
            surge_indices = np.where(surge_mask)[0]
            surge_close_change = 0.0
            for si in surge_indices:
                if si > 0 and cls[si] > cls[si - 1]:
                    surge_close_change = max(surge_close_change, cls[si] / cls[si - 1] - 1)

            total_change = cls[-1] / cls[0] - 1 if cls[0] > 0 else 0

            if surge_close_change > 0.03 and total_change < 0.50:
                ab = 5.0
            elif surge_close_change > 0.02 and total_change < 0.50:
                ab = 4.0
