...
def cmd_preselect(args: argparse.Namespace) -> None:
    ...
    label = f"（策略={strategy}）" if strategy else ""
    logger.info("===== 量化初选开始 %s =====", label)
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--strategy", default=None, choices=["b1", "brick"],
                   help="仅运行指定策略（默认按配置文件运行所有启用策略）")
